...
function main() {
	...
	function decode() {
		...
		void main ()
		{
			...
			uvec4 s_rev = uvec4(${searched[0]}u, ${searched[1]}u, ${searched[2]}u, ${searched[3]}u) - uvec4(MD5_A, MD5_B, MD5_C, MD5_D);
			...
			for (uint i = 0u; i < ${innerLoopCnt}u; i++) {
				...
				w0.x += 1u;
				if ((w0.x & 0xffu) == ${charset.afterLastChar}u) { // w0 first char
					w0.x += ${charsOffset}u;
					if ((w0.x & 0xff00u) == ${charset.afterLastChar << 8}u) { // w0 second char
						w0.x += ${charsOffset << 8}u;
						if ((w0.x & 0xff0000u) == ${charset.afterLastChar << 16}u) { // w0 third char
							w0.x += ${charsOffset << 16}u;
						}
					}
				}
				uint w0_first_chars = w0.x & ${w0FirstCharsMask}u;
				w0 &= ${w0RestCharsMask}u;
				w0 |= w0_first_chars;
			}
		}
		`;
